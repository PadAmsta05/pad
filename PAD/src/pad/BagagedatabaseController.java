...
public class BagagedatabaseController implements Initializable {
    ...
    public void getLuggageData() {
        ...
        try {
            conn = pad.connectToDatabase(conn);
            ...
            try (ResultSet rs = stmt.executeQuery(sql)) {
                while (rs.next()) {
                    ...
                    int id = rs.getInt("id");
                    String status = pad.getStatus(rs.getInt("status"));
                    String type = pad.getBaggageType(rs.getInt("type"));
                    String color = pad.getColor(rs.getInt("color"));
    ...
    private void handleSendToDatabase(ActionEvent event) throws IOException, SQLException {
        if ((typeCombo.getValue() == null)
            ...
            if (pad.checkEmailExistsOnChange(mailInput.getText(), dr_mail)) {
                ...
                sendToDatabase(Integer.parseInt(idLabel.getText()),
                        ...
                        pad.getStatusString(statusCombo.getValue().toString()),
                        ...
                        pad.getBaggageTypeString(typeCombo.getValue().toString()),
                        brandInput.getText(),
                        pad.getColorString(colorCombo.getValue().toString()),
    ...
    private void sendToDatabase(int dr_id, int dr_personId, int dr_lafId,
        ...
        try {
            ...
            if (status != pad.getStatusString(dr_status)) {
            ...
            if (status != 3) {
                int pageid = 4;
                int type_email = 0;
                if (mailInput.getText() == null || !PAD.isValidEmailAddress(mailInput.getText()) || mailInput.getText().trim().isEmpty()) {

                } else {
                    // Email bericht filteren op sommige woorden.
                    String getmessage = pad.replaceEmail(pad.replaceEmail_tF(pad.Email_Message(type_email,
                            pad.Email_Language(mailInput.getText()), pageid), mailInput.getText(),
                            Integer.parseInt(tableFromLabel.getText())), mailInput.getText());
                    // Email versturen
                    pad.sendEmail(mailInput.getText(), pad.Email_Subject(type_email, pad.Email_Language(mailInput.getText()), pageid), getmessage, "Sent message successfully....");
                }
            }
            ...
            if (status == 4) { //Nooit gevonden
                ...
                String email_paymentdepartment = "padepsilon@gmail.com"; // Betalingsafdeling mail
                if (mailInput.getText() == null || !PAD.isValidEmailAddress(mailInput.getText()) || mailInput.getText().trim().isEmpty()) {

                } else {
                    // Email bericht filteren op sommige woorden.            
                    String getmessage = pad.replaceEmail(pad.replaceEmail_tF(pad.Email_Message(type_email, language_email, pageid), mailInput.getText(), Integer.parseInt(tableFromLabel.getText())), mailInput.getText());
                    // Email versturen
                    pad.sendEmail(email_paymentdepartment, pad.Email_Subject(type_email, language_email, pageid), getmessage, "Sent message successfully....");
                }
            }
    ...
    public void handleFileSelector(ActionEvent event) throws FileNotFoundException, IOException {
        File file = pad.fileChooser();
        //String fileRaw = file.getAbsolutePath();
        filePath = "pad/src/pad/luggageImages/" + file.getName();
