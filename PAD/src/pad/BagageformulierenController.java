...
import static pad.PAD.generateRandomPassword;
import java.io.File;
...
public class BagageformulierenController implements Initializable {
    ...
    public String password = PAD.encrypt(generateRandomPassword(8));
    ...
    private void handleSendToDatabase(ActionEvent event) throws IOException, SQLException {
        ...
        if ((name_input.getText() == null || name_input.getText().trim().isEmpty())
        ...
        } else if (!pad.isValidEmailAddress(mail_input.getText())) {
            //Indien het ingevulde emailadres al in de database bestaat foutmelding geven
            loginerror.setVisible(false);
            loginerror.setText(taal[159]);
            loginerror.setStyle("-fx-text-fill: red;");
            loginerror.setVisible(true);
        } else if (pad.checkEmailExists(mail_input.getText())) {
            //Indien het ingevulde emailadres al in de database bestaat foutmelding geven
            loginerror.setVisible(false);
            loginerror.setText(taal[121]);
            loginerror.setStyle("-fx-text-fill: red;");
            loginerror.setVisible(true);
        } else if (pad.checkFound(
                pad.getBaggageTypeString(type_combo.getValue().toString()),
                brand_input.getText(),
                pad.getColorString(color_combo.getValue().toString()))) {
            popup.setVisible(true);
            formulier.setDisable(true);

            int count = pad.countFound(pad.getBaggageTypeString(type_combo.getValue().toString()), brand_input.getText(), pad.getColorString(color_combo.getValue().toString()));
             if (count == 1) {
            ...
            sendToDatabase(airport_combo.getValue().toString(), name_input.getText(),
                    ...
                    pad.getBaggageTypeString(type_combo.getValue().toString()),
                    brand_input.getText(), pad.getColorString(color_combo.getValue().toString()),
    ...
    public void getLuggageData() {
        try {
            ...
            while (rs.next()) {
                ...
                String statussql = pad.getStatus(rs.getInt("status"));
                String typesql = pad.getBaggageType(rs.getInt("type"));
                String colorsql = pad.getColor(rs.getInt("color"));
    ...
    private void sendToDatabase(String airport, String frontname, String surname,
        ...
        try {
            ...
            String sql_person = "INSERT INTO bagagedatabase.person (type, language, "
                    ...
                    + pad.getUserLanguageString(language_combo.getSelectionModel().getSelectedItem().toString())
                    + "', '" + frontname + "', "
            ...
            String getmessage = pad.replaceEmail(pad.Email_Message(type_email, 
                    pad.getUserLanguageString(language_combo.getValue().toString()), 
                    pageid), mail_input.getText());
            // Email versturen
            pad.sendEmail(mail_input.getText(), pad.Email_Subject(type_email, 
                    pad.getUserLanguageString(language_combo.getValue().toString()), pageid), 
    ...
    public void handleFileSelector(ActionEvent event) throws IOException {
        ...
        filePath = "/pad/src/pad/luggageImages/" + (file == null ? "" : file.getName());
